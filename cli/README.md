...
- **5~8스텝 적정(최소 4, 최대 10), 중요한 순서대로** — 필름은 ~30초라 뒤부터 잘립니다.
...
- 스텝마다 `action`과 `selector`(모르면 `where`)를 **같이** 넣으세요 — `goal`만 있는 스텝은 목차일 뿐이라 서버가 거절합니다(최소 3스텝이 이 조건을 채워야 함).
- `hold`(0.5~4초): 천천히 봐야 하는 스텝의 결과를 그만큼 더 보여줍니다.
- 대본에 있어도 로봇은 **로그인·제출·삭제·파일 선택은 누르지 않습니다.**
- 발행 결과에 **대본 점검** 한 줄이 붙습니다 — 스텝 수·실제 조작 수·셀렉터/기대값 채움, 그리고 서버가 진입 URL의 HTML을 한 번 받아
  **셀렉터가 실제로 있는지** 센 결과. `⚠`가 붙은 줄은 고치고 같은 URL로 다시 `publish`하면 초안이 갱신됩니다.
  (자바스크립트가 그리는 화면은 서버가 못 보니 그땐 브라우저로 직접 확인하라고 알려줍니다.)
