...
export async function draftsCommand(args) {
  ...
  if (sub === "update") {
    // JSON(--file·표준입력·--json) 위에 플래그를 덮는다 — publish와 같은 규칙.
    const payload = (await readJsonObject(args)) ?? {};
    if (args.title) payload.title = args.title;
    if (args.description) payload.description = args.description;
    if (args.note) payload.builderNote = args.note;
    if (args.hint) payload.demoHighlights = args.hint;
    if (!Object.keys(payload).length) {
      throw new Error("Nothing to change — pass --title/--description/--note/--hint, or JSON via --file/--json.");
    }
