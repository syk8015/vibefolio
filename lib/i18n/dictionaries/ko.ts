...
export const ko = {
  ...
  connect: {
    ...
    copyPrompt: "프롬프트 복사",
    copying: "토큰 발급·복사 중…",
    copiedNote: "이 버튼을 다시 누르면 이전 토큰은 자동 폐기돼요.",
    ...
    // 셸 없는 챗봇용 출구(2026-09-18). 이 카드는 "AI가 스스로 올려줄 것"을 전제로
    // 기다리는데, 채팅창 AI는 영원히 안 올린다 — JSON을 손에 든 사람이 붙여넣을
    // 곳을 못 찾고 멈추던 자리가 정확히 여기다.
    pasteJsonLead: "AI가 JSON만 주고 끝났나요?",
    ...
    mcpToggle: "MCP로 연결하기 — 붙여넣기가 아예 없어져요",
    mcpLead: "클로드코드·클로드 데스크탑·커서에 Nookframe을 MCP로 붙이면 \"이거 Nookframe에 올려줘\" 한마디로 끝나요. 프롬프트도, JSON도 옮길 필요가 없어요.",
    // 원격 커넥터(2026-09-17) — 설치도 토큰 이동도 없는 유일한 길이라 맨 위에 둔다.
    mcpRemoteTitle: "클로드 웹·데스크탑 앱 — 주소만 붙여넣기",
    mcpRemoteHint: "클로드에서 설정 → 커넥터 → [커스텀 커넥터 추가]에 이 주소를 넣고, 뜨는 안내대로 [허용]을 누르면 끝이에요. 설치할 것도, 토큰을 옮길 일도 없어요.",
    ...
    mcpCopy: "토큰 채워서 복사",
    mcpCopying: "토큰 발급 중…",
    mcpCopied: "복사했어요 — 토큰이 채워져 있으니 그대로 붙여넣으세요. 다시 복사하면 이전 MCP 토큰은 폐기돼요.",
    mcpTokenName: "MCP 연결",
    revoke: "폐기",
  },
