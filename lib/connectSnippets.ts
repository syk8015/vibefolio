...
export function outputLanguageLine(locale: "ko" | "en"): string {
  ...
  return "LANGUAGE — this owner's page is Korean, so write every human-readable string you produce (title, description, builderNote, notes) in KOREAN. Do NOT translate JSON keys, enum values (contentType, targetDevice, action names) or tag spellings — those stay exactly as written below.";
}
