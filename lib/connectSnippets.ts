...
3) 아래 항목을 채운 publish 페이로드(JSON)를 만들어:
   ...
   • deployUrl — 배포된 공개 URL (vercel/netlify 등). 미배포면: 우선 정적 빌드 산출물을 올려(백엔드 없는 앱에 제일 안전). 그걸로 안 되는 앱(서버·DB 필요)이면 공개 GitHub 저장소 URL을 대신 줘도 돼 — clone 후 자동 실행하는 최후 수단이야(JS 리포는 npm run dev/start, 파이썬 웹앱은 Streamlit·Gradio·Flask·FastAPI·Dash를 감지해 pip install 후 실행. 웹 화면이 아예 없는 프로젝트(CLI 도구·봇·백엔드)는 라이브 터미널을 띄워 로봇이 명령어를 쳐 보는 영상으로 촬영돼 — 이 경우 demoScript에 정확한 명령어를 적어주면 영상이 훨씬 좋아져. 비공개 저장소는 실패, 원격 DB 쓰는 앱은 읽기전용 데모로 나옴)
