...
| demoAccessRequired (09-15에 바꾼 문장만) | 데모/게스트 모드가 없다면 가짜 데이터로 작게 하나 만드는 게 이 영상에 가장 좋은 투자예요. 다만 주인의 앱을 고치는 일이니 코드를 쓰거나 배포하기 전에 주인에게 먼저 물어보세요. | if no demo/guest mode exists yet, a small one with fake data is the single best thing for this film, but it changes the owner's app: ask the owner before writing that code or deploying. | 전문은 사전 참조 · "먼저 묻기" 추가(외부 AI가 코드 수정·운영 배포로 번진 사례) |
| demoAccessEvidence (noLogin, 09-15에 바꾼 문장만) | 없다면 가짜 데이터로 작게 하나 만드는 게 이 영상을 위한 가장 값진 투자예요(코드를 고치거나 배포하기 전에 주인에게 먼저 물어보세요). | and if none exists, a small one with fake data is the best investment for this film (ask the owner before changing their code or deploying). | 함수(which) · 전문은 사전 참조 |
...
| api.scriptReview.filmTooLong | 이 대본은 촬영에 약 {seconds}초가 걸리는데 필름은 약 {budget}초에서 끊겨요 — {cut}번째 스텝부터는 영상에 못 들어가요. 덜 중요한 비트를 빼거나(5~8스텝이 알맞아요) hold를 줄이세요. … | This script is roughly {seconds}s of filming, but the camera stops at about {budget}s — step {cut} and everything after it would not make the film. … | 신규 · 예상 필름 길이(09-16 NF-05/17), 함수(seconds, cut, budget) |
| api.newDraftConflict | newDraft와 draftId는 서로 반대예요 — newDraft는 늘 새 초안을 만들고, draftId는 그 초안을 갱신해요. 둘 중 하나만 보내세요. | newDraft and draftId are opposites — newDraft always creates a new draft, draftId updates that one. Send only one of them. | 신규 · 400 BAD_REQUEST(09-16 NF-16 별도 생성 옵션) |
