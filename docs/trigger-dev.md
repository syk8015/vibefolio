# Trigger.dev v4 reference (moved from AGENTS.md)

Tool-injected docs (trigger.dev CLI agent-rules installer), relocated 2026-07-05 to keep the session-start payload small. Read this before editing `src/trigger/*`, `trigger.config.ts`, or anything importing `@trigger.dev` (currently `app/api/projects/[id]/trigger-demo/route.ts`, `scripts/trigger-build.mjs`).

Note: the Realtime section below documents features this repo does not use (`@trigger.dev/react-hooks` is not installed; run status is mirrored into the Supabase `projects` table instead of Realtime subscriptions).
