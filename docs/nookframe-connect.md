...
09-17 실측에서 **아티팩트 링크는 봇 검사에 막혀 촬영이 아예 불가능**하다는 게 드러나, 그
사람들에게 남은 길이 "사람이 손으로 내려받아 `/publish`에 올리기" 하나뿐이 됐다. 그 4단계를
없앤 것이 `htmlBody`다 — payload에 **파일 하나짜리 작품의 HTML 전문**을 실어 보내면 된다.

구현은 `lib/htmlBody.ts` 하나이고, 라우트가 하는 일은 **본문 파싱 직후 그것을 `index.html`
하나짜리 zip으로 바꿔 `bundle` 변수에 넣는 것**뿐이다(3.2단계). 그 아래 코드는 한 줄도 안
바뀐다 — 번들 유무 판정·저장·교체 시 옛 파일 정리가 그대로 적용된다. **전용 저장 경로를
새로 내지 않는 이유**: 서버 zip은 서비스롤이라 스토리지 RLS를 우회하므로 경로 검사와
prefix assert가 유일한 방어인데, 사본을 만들면 그 방어가 갈라진다(AGENTS.md 불변식).

입구 검사 4종(`htmlBodyIssue`) — `empty` · `too-large`(2MB) · `not-html`(조각·설명문) ·
**`truncated`**. 마지막이 이 길의 핵심이다: 긴 코드는 AI 답변이 중간에 끊기는데, 그대로
저장되면 반쪽짜리 작품이 "발행 성공"으로 올라가고 사람은 영상을 보고서야 안다. 닫는
`</html>`도 `</body>`도 없으면 거절한다. ```` ```html ```` 울타리째 와도 벗겨서 받는다.
검증=`npm test`의 `probe-html-body-unit`(20단언 — 만든 zip이 실제로 `expandZipBundle`·
`pickZipAnchor`를 통과하는 것까지).

한계: **파일 하나짜리 전용**이다. CSS·JS가 따로 있는 작품은 여전히 사람이 zip으로 올려야
한다(여러 파일을 글자 한 덩이로 되돌리는 표준 포맷이 없다).
