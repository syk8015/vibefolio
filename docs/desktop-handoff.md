...
**폰에서 바로 가입한 사람 (09-25)**
- 로그인 화면의 Google·GitHub·메일 코드, `/send`의 "여기서 가입" 등 폰에서 가입을 끝내는 길은 여럿이다.
  어느 길이든 온보딩(아이디 정하기)을 거치므로, **온보딩이 끝날 때 폰 폭(768px 미만)이면 `/send`로** 보낸다
  (?next=가 대시보드가 아닌 곳이면 그대로 — /publish에서 온 사람 등).
- 로그인한 사람이 보는 `/send`는 "가입 완료! 이제 컴퓨터에서 이어서 해요" + 계정 이메일(서버가 채움) + 버튼 하나.
  입력·로봇 확인 없음. [폰으로 먼저 둘러볼게요 →]로 대시보드에 갈 수 있다(막지 않는다).
- 서버는 `self: true`면 **받는 주소를 계정 이메일로만** 정한다(몸통의 email 무시) → 남에게 보낼 수 없어 Turnstile 생략.
- 메일 링크는 같은 `/signup?h=` — 메일 코드 로그인이 있는 계정이면 들여보낸다. 그래서 가입 화면·메일 문구를
  "가입" 대신 "이메일이 채워진 화면·코드로 들어가기"로 두 경우에 다 맞게 바꿨다.
