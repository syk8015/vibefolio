...
import { firstTouch, adoptHandoffTouch, type FirstTouchData } from "@/lib/analytics-client";
...
// 메일 링크로 온 사람(서버가 찾아 준 값). null이면 평소 가입 화면.
...
export default function SignupForm({ handoff }: { handoff: SignupHandoff | null }) {
  const { t, locale } = useT();
  const router = useRouter();
  const [show, setShow] = useState(false);
  // 비밀번호 없이 메일 코드로 가입하는 모드(EmailCodeForm — 로그인 화면과 같은 폼).
  // 메일 링크로 왔으면 이메일은 이미 안다 → 처음부터 코드 가입.
  const [mode, setMode] = useState<"password" | "code">(handoff ? "code" : "password");
  // 메일 링크로 왔으면 Google·GitHub 버튼을 접어 둔다 — 이메일 칸이 아래로 밀려 보이지
  // 않게(09-24 사용자 지시). 아래 링크로 다시 펼 수 있다. 평소 가입 화면은 그대로.
  const [showSocial, setShowSocial] = useState(!handoff);
  const [step, setStep] = useState<Step>("form");
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState("");
  const [form, setForm] = useState({ email: handoff?.email ?? "", password: "" });
  ...
  // 폰 → 컴퓨터 넘기기(docs/desktop-handoff.md): 메일 링크 /signup?h=<id>로 왔으면 서버
  // (page.tsx)가 이메일을 채워 보냈다. 여기선 폰의 광고 출처를 이 브라우저 first-touch로
  // 옮기고(가입 메타데이터·온보딩 signup_completed가 싣는다), "열림"을 서버에 찍는다.
  useEffect(() => {
    if (!handoff) return;
      ...
      <div className="flex-1 flex items-center justify-center px-6 py-12">
        <div className="w-full max-w-sm">
          <div className="mb-8">
            ...
            <p className="text-sm font-semibold" style={{ color: "var(--text-secondary)", fontFamily: "var(--font-nunito)" }}>
              {handoff ? t.signup.handoffSubtitle : t.signup.subtitle}
            </p>
          </div>
          ...
          {!showSocial && (
            <p className="text-center text-xs mt-6" style={{ fontFamily: "var(--font-nunito)" }}>
              <LinkButton onClick={() => setShowSocial(true)}>{t.signup.otherWays}</LinkButton>
            </p>
          )}
