...
// "열림"을 찍는 일을 한다. 응답의 이메일·first-touch는 찔러보기가 읽기 규칙을 확인하는 데 쓴다.
...
export async function POST(req: NextRequest) {
  try {
    ...
    const row = await findLiveHandoff(id);
    ...
    return NextResponse.json({ ok: true, email: row.email, firstTouch: row.firstTouch });
  } catch (err) {
