// 로그인 방법 연결(2026-09-25, 명함 탭 "로그인 방법") prod E2E. 구글·깃허브 화면까지는 안 간다.
//
...
const RETURN_PATH = "/dashboard?tab=card";
...
// (3) 콜백 — 계정이 필요 없다(결과 싣기는 코드 교환 전에 갈린다).
{
  ...
  ok("콜백: 남의 계정에 붙은 깃허브 → 명함 탭 link=taken", loc === `${back}&link=taken&provider=github`, loc);

  loc = await cb(`${LINK}&error=access_denied&${desc("The user has denied your application access.")}`);
  ok("콜백: 공급자 화면에서 취소 → 명함 탭 link=failed", loc === `${back}&link=failed&provider=github`, loc);

  loc = await cb(`${LINK}&code=00000000-0000-0000-0000-000000000000`);
  ok("콜백: 코드 교환 실패 → 명함 탭 link=failed", loc === `${back}&link=failed&provider=github`, loc);
